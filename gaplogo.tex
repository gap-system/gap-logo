...
\usepackage[svgnames]{xcolor}
  ...
  \definecolor{dynkincolor}{named}{Indigo}
...
\begin{tikzpicture}[line width=\lw, >={Stealth[inset=2pt, angle=75:13pt, bend]}]
  ...
  \draw[<->, MediumOrchid] ({0+\margin}:\radius) 
    arc ({0+\margin}:{120-\margin}:\radius);

  \draw[<->, DarkOrchid] ({120+\margin}:\radius) 
    arc ({120+\margin}:{240-\margin}:\radius);

  \draw[<->, Orchid] ({240+\margin}:\radius) 
    arc ({240+\margin}:{360-\margin}:\radius);

\ifx\NoTextMode\undefined
    % draw the GAP text
    \node[anchor=west] at (1.2, 0.8) {\gapstyle{\textcolor{MediumOrchid}{G}}\extrastyle{roups}};
    \node[anchor=west] at (1.2, 0  ) {\gapstyle{\textcolor{DarkOrchid}{A}}\extrastyle{lgorithms}};
    \node[anchor=west] at (1.2,-0.8) {\gapstyle{\textcolor{Orchid}{P}}\extrastyle{rogramming}};
\fi
