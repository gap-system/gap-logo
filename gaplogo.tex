...
\begin{tikzpicture}[line width=\lw, >={Stealth[inset=2pt, angle=75:13pt, bend]}]
  ...
  \draw[<->, DarkSeaGreen] ({0+\margin}:\radius) 
    arc ({0+\margin}:{120-\margin}:\radius);

  \draw[<->, CadetBlue] ({120+\margin}:\radius) 
    arc ({120+\margin}:{240-\margin}:\radius);

  \draw[<->, DarkCyan] ({240+\margin}:\radius) 
    arc ({240+\margin}:{360-\margin}:\radius);

\ifx\NoTextMode\undefined
    % draw the GAP text
    \node[anchor=west] at (1.2, 0.8) {\gapstyle{\textcolor{DarkSeaGreen}{G}}\extrastyle{roups}};
    \node[anchor=west] at (1.2, 0  ) {\gapstyle{\textcolor{CadetBlue}{A}}\extrastyle{lgorithms}};
    \node[anchor=west] at (1.2,-0.8) {\gapstyle{\textcolor{DarkCyan}{P}}\extrastyle{rogramming}};
\fi
